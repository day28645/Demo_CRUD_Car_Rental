...
{
    public partial class ManageBookingList : System.Web.UI.Page
    {
        ...
        {
            if (e.CommandName == "pick_car")
            {
                var book_id = e.CommandArgument.ToString();
                if (!string.IsNullOrEmpty(book_id))
                {
                    Response.Redirect($"~/Page_Employee/UpdatePick.aspx?booking_no={book_id}");
                }
            }
            else if (e.CommandName == "return_car")
            {
                var book_id = e.CommandArgument.ToString();
                if (!string.IsNullOrEmpty(book_id))
                {
                    Response.Redirect($"~/Page_Employee/UpdateReturn.aspx?booking_no={book_id}");
                }
            }
            else if (e.CommandName == "view_car")
            {
                var book_id = e.CommandArgument.ToString();
                if (!string.IsNullOrEmpty(book_id))
                {
                    Response.Redirect($"~/Page_Employee/DisplayBooking.aspx?booking_no={book_id}");
                }
            }
        }
